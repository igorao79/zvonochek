...
class ResilientChannelManager {
    ...
    return new Promise((resolve, reject) => {
      const createChannel = () => {
        try {
          ...
          // Подписываемся с callback функцией
            ...
            switch (status) {
              ...
              case 'CHANNEL_ERROR':
                ...
                if (isWebRTCOrCallChannel && (
                  ...
                  console.log(`🔇 [ResilientChannel] Suppressed expected error in ${channelName}: ${errorMsg}`)
                  return
                }

                console.error(`❌ [ResilientChannel] Channel error in ${channelName}:`, errorMsg)
              ...
              case 'TIMED_OUT':
                console.warn(`⏱️ [ResilientChannel] Timeout in ${channelName}`)
                channelState.isHealthy = false
              ...
              default:
                console.log(`ℹ️ [ResilientChannel] Channel status in ${channelName}: ${status}`)
            }
          })
  ...
  private async recreateChannel(channelName: string) {
    ...
    try {
      ...
      // Подписываемся заново с callback функцией
      configuredChannel.subscribe((status, err) => {
        switch (status) {
          case 'SUBSCRIBED':
          ...
          case 'CHANNEL_ERROR':
            ...
            console.error(`❌ [ResilientChannel] Reconnection error in ${channelName}:`, errorMsg)
            channelState.isHealthy = false
            channelState.errorCount++
          ...
          case 'TIMED_OUT':
            console.warn(`⏱️ [ResilientChannel] Reconnection timeout in ${channelName}`)
            channelState.isHealthy = false
            break
          ...
          default:
            console.log(`ℹ️ [ResilientChannel] Reconnected channel status in ${channelName}: ${status}`)
        }
      })
    } catch (error) {
  ...
  // Добавление существующего канала в менеджер (без создания устойчивого)
  addChannel(channel: RealtimeChannel, channelName: string) {
    if (!channel || !channelName) return

    // Проверяем, что канал уже не управляется менеджером
    if (this.channels.has(channelName)) {
      console.log(`⚠️ [ResilientChannel] Channel ${channelName} already managed, skipping add`)
      return
    }

    console.log(`➕ [ResilientChannel] Adding existing channel: ${channelName}`)

    // Создаем минимальный state для канала
    const channelState: ChannelState = {
      channel,
      config: {
        channelName,
        setup: () => channel, // Пустая функция setup
      },
      reconnectionManager: null, // Без переподключения
      lastActivity: Date.now(),
      errorCount: 0,
      isHealthy: true,
      isReconnecting: false,
